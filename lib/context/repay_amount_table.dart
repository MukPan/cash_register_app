import 'package:cash_register_app/provider/repay_money_count_family.dart';
...
class RepayAmountTable extends HookConsumerWidget {
  ...
  List<DataRow> _getRepayCountRows(WidgetRef ref) {
    return denominationInfoList.reversed
        .where((info) => ref.read(repayMoneyCountFamily(info.denominationType)) != 0) //0枚をリストから除外
        .map((info) {
      final int changeCount = ref.read(repayMoneyCountFamily(info.denominationType));
      return DataRow(
          cells: [
